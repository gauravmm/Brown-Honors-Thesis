...
\\ & = \prod_{\lambda \in \Lambda_j} \left( (1 - \alpha) * \frac{Q(\gamma, \lambda)}{\sum_{\omega} Q(\omega, \lambda)} + \frac{\alpha}{\xi} \right) \end{align*}

In the latter case, we convert the expression to a combination of a spatial model of a preposition and at least one other distribution over objects. Details of this process are in Section \ref{sec:bottomupeval}; Equation~\ref{eqn:prepositions} is reproduced here:
\begin{align*}
   & \Pr(\Lambda_j | \Gamma = \gamma)
\\ & \propto \sum_{\gamma_1 \in \xi} \cdots\sum_{\gamma_n \in \xi} \left(\prod_{i=1}^n \Pr(\Lambda_{j,i}|\Gamma_{j,i}) \right) \cdot \text{S}\left( \frac{f(\gamma, \gamma_1, \ldots) \cdot \theta_p}{z} \right)
\end{align*}
...
\Tree [.{$\Pr(\Gamma|\Lambda)$} \emph{The orange cube} [.{\emph{between}} \emph{the red} \emph{the yellow} ]] \vspace{0.12in} &
...
We have preposition $p \in P = $ \{`near', `left', `right', `front', `behind', `between'\}. The $i^\text{th}$ grounding that the preposition relies on is modeled by the distributions $\Pr(\Lambda_{j,i}|\Gamma_{j,i})$, obtained from the language model (See Case 0). We find $\Pr(\Lambda_j | \Gamma = \gamma)$ with:

\break

\begin{align}
   & \Pr(\Lambda_j | \Gamma = \gamma)
\intertext{Use the structure of prepositional phrases to factor $\Lambda_j$ into a preposition and groundings. $\Lambda_{j,P}$ refers to the subset of $\Lambda_{j}$ which describes the preposition (e.g. the word ``between'' or ``near''), and corresponds to $P$, the distribution over possible prepositions. Each $\Lambda_{j,i}$ refers to the subset of $\Lambda_{j}$ which describes the $i^\text{th}$ object that the preposition references (i.e. the object to which the target is ``near'') and corresponds to the distribution over all objects $\Gamma_{j,i}$. Let $n$ be the number of such groundings.}
\begin{split}
& = \sum_{p \in P} \sum_{\gamma_1 \in \xi} \cdots\sum_{\gamma_n \in \xi}
\\ & \phantom{=}~ \Pr(\Lambda_{j,P},\Lambda_{j,1}, \ldots, \Lambda_{j,n}| P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n)
\\ & \phantom{=}~\cdot \Pr(P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n | \Gamma = \gamma)
\end{split}
\intertext{We assume that each grounding is independent, which allows us to separate each grounding in the first inner term:}
\begin{split}
& = \sum_{p \in P} \sum_{\gamma_1 \in \xi} \cdots\sum_{\gamma_n \in \xi}
\\ & \phantom{=}~ \Pr(\Lambda_{j,P} | P = p) \cdot \left(\prod_{i=1}^n \Pr(\Lambda_{j,i}|\Gamma_{j,i} = \gamma_i) \right)
\\ & \phantom{=}~\cdot \Pr(P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n | \Gamma = \gamma)
\end{split}
\intertext{We assume that prepositions are certain, and so eliminate the associated term and summation.}
\begin{split}
& = \sum_{\gamma_1 \in \xi} \cdots\sum_{\gamma_n \in \xi}
\\ & \phantom{=}~ \left(\prod_{i=1}^n \Pr(\Lambda_{j,i}|\Gamma_{j,i} = \gamma_i) \right) 
\\ & \phantom{=}~ \cdot \Pr(P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n | \Gamma = \gamma)\label{eqn:prep_model_pending}
\end{split}
\end{align}
...
\begin{align}
& \Pr(P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n | \Gamma = \gamma)
\intertext{Applying Bayes' theorem:}
\begin{split}
& = \Pr(\Gamma = \gamma | P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n) 
\\ & \phantom{=}~\cdot \frac{\Pr(P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n)}{\Pr(\Gamma = \gamma)}
\end{split}
\intertext{Since we have already assumed that prepositions are certain and that the prior probability of all groundings is uniform over all objects, we can simply this to:}
& \propto \Pr(\Gamma = \gamma | P = p, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n) 
\intertext{We parametrize this with feature-vector function $f$, weights $\theta_p$, logistic function $S$, and some normalization factor $z$:}
   & = \text{S}\left( \frac{f(\gamma, \gamma_1, \ldots) \cdot \theta_p}{z} \right) \label{eqn:prep_model_inner}
\end{align}

We can estimate $\theta_p$ for each $p$ using logistic regression, and select feature-vector function $f$ separately.

We substitute the spatial model in Equation~\ref{eqn:prep_model_inner} into Equation~\ref{eqn:prep_model_pending}, and obtain:
\begin{align}
   & \Pr(\Lambda_j | \Gamma = \gamma) \nonumber
\\ 
& \propto \sum_{\gamma_1 \in \xi} \cdots\sum_{\gamma_n \in \xi} \left(\prod_{i=1}^n \Pr(\Lambda_{j,i}|\Gamma_{j,i}) \right) \cdot \text{S}\left( \frac{f(\gamma, \gamma_1, \ldots) \cdot \theta_p}{z} \right) \label{eqn:prepositions}
\end{align}
