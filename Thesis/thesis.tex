...
We borrow the application of correspondence variables from work by \citet{tellex2011understanding}. We reformulate the problem with correspondence variable $\Phi$, which is true if and only if the object $\gamma$ is correctly identified:
\begin{align}
...
We have preposition $p \in P = $ \{`near', `left', `right', `front', `behind', `between'\}. The $i^\text{th}$ grounding that the preposition relies on is modeled by the distributions $\Pr(\Lambda_{j,i}|\Gamma_{j,i})$, obtained from the language model (See Case 0). In this case, we model We find $\Pr(\Lambda_j | \Gamma = \gamma)$ with:
...
\begin{align}
& \Pr(\Gamma = \gamma, \Lambda_i | \phi_i) \nonumber
\\ & = \Pr(\Gamma = \gamma, \Lambda_{j,P},\Lambda_{j,1}, \ldots, \Lambda_{j,n}| \phi_i)
\\ \begin{split}
& = \sum_{\gamma_1 \in \xi} \cdots\sum_{\gamma_n \in \xi}
\\ & \phantom{=}~ \Pr(\Lambda_{j,P},\Lambda_{j,1}, \ldots, \Lambda_{j,n}| \Gamma = \gamma, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n)
\\ & \phantom{=}~\cdot \Pr(\Gamma = \gamma, \Gamma_{j,1} = \gamma_1, \ldots, \Gamma_{j,n} = \gamma_n | \phi_i)
\end{split}
...
We estimate $\theta_p$ for each $p$ using logistic regression, with the feature-vector functions detailed in \ref{sec:featurevectors}.
...
\item The product of the distances: $f_1(p, q) = (|p| +\epsilon)\cdot(|q| + \epsilon)$, with arbitrary $\epsilon = .1$.
\end{enumerate}
\end{flushleft}
